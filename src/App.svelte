...
export let puzzle, checkTiles, finished;
...
<main class={'puzzle' + (finished ? ' finished' : '')}>
	...
	<div class="shelf" id="shelf-{puzzle.name}">
	{#each puzzle.shelf as tile}
	<div class="tile" id="tile-{puzzle.name}-{tile.name}" bind:this={puzzle.tileTargets[tile.name]} style="left: {tile.x * 100}%; top: {tile.y * 100}%; width: {tile.shape.width * 100}%; height: {tile.shape.height * 100}%">{tile.label}</div>
...
<style>
	...
	.shelf {position: relative; width: 100%; height: 10em; background-color: aliceblue;}
